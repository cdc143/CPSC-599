...
titleScreen:
 ...
 sta temp_colour
 jsr loadLevel1 	;load level 3 to screen
...
iterateLevelLoopDrawWall:		;space
 lda space_sprite
 sta level_char
 rts
...
drawRow:		;draws 3 rows because 22/7 bits = 3 blocks per bit with one extra 
 ...
 jsr iterateLevelLoopDrawCharacter
 inc row
 jsr iterateLevelLoop
 jsr iterateLevelLoopDrawCharacter
 inc row
...
drawRowLoop:
 jsr drawRow		;draws 3 rows to screen
 lsr current_level_part		;shift one bit right
 lda row
...
leftBot:
 ...
 sta col_bot
updateLeft:
 cpx row_col_begin				; check if end of screen on left
...
right:			;similar as above left subroutine
 ...
 bpl rightBot
rightCheck:
 cpx col_mid_up_row_mid_right		 	; check if x >13
 bmi updateRight
rightBot:
 lda #$01			;set col_bot flag
 sta col_bot
...
up:
 ...
 sta col_bot
 ldx col			;top cols
 cpx row_col_begin		 ; check if x < 21 (on top row)
 beq updateNewLoc	 ; at end of screen top
 cpx col_mid_up_row_mid_right			;else, check if in top half of screen
