...
;changes the border and background char_colour -> page 175 of vic do manual
 ...
 iny
 ;jsr loadLevel3
 jsr loadLevel	;just loads a random level 
 ldx init_lives
 stx lives
 jmp initChars
...
loadLevel1:
 lda current_room	;load current room
 cmp #$00		;if room is level 1 room
 bne loadLevel1End		;not current room
...
loadLevel2:
 lda current_room
 cmp #$01		;if room is level 2 room
 bne loadLevel2End
 ldx #$00
loadLevel2Loop1:
 lda level2top,x
...
loadLevel3:
 lda current_room
 cmp #$02		;if room is level 2 room
 bne loadLevel3End
 ldx #$0
loadLevel3Loop1:
 lda level3top,x
...
loadLevel4:
 lda current_room
 cmp #$03		;if room is level 2 room
 bne loadLevel4End
 ldx #$00
loadLevel4Loop1:
 lda level4top,x
...
initCharsNextLevel:	
 ldx init_lives
 jsr drawLives
 lda $9005		; load character memory ;this code needs to be here in order to draw the screen properly
 pha			; push acc onto stack
 ora #$0f		; set character memory (bits 0-3)
 sta $9005 		; store result
...
collision:		 ; detect collision between B and C
 ...
 beq drawcoll
 cpy door_sprite		;for some reason this doesn't work for all doors?
 bne drawCharacter1
 jsr $e55f		;clear screen
 jsr loadNewLevel
 ;jmp initCharsNextLevel		;taken care of in loadNewLevel
 ;jmp initChars
drawCharacter1:
 jsr checkEndScreen		;check if character is going off screen
...
loadNewLevel:	;I mean no offense by commenting much of the code out, just trying to test to see which parts 
				;aren't working.  Once we figure that out, then the algorithm can be finished! :)
  jsr levelDispatch	 ;just jumps 
  rts
checkEndScreen:			;checks conditions for walking off end of screen
  lda col		
  cmp col_end			;doesn't work
  beq yesNewLevel
  cmp col_begin			;this works
  beq yesNewLevel
  lda row
  cmp row_begin			;this works for top half of screen only
  beq yesNewLevel
  cmp row_end			;this works for top half of screen only
  beq yesNewLevel
  bne noNewLevel
yesNewLevel:
 jsr levelDispatch
...
levelDispatch:		;this just gets a random room for the moment just for testing.
 ;ldy current_room
 ;lda rooms,y
 ;cmp #$04
 jsr getRandom		;random number
 and #$03			;4 rooms to choose from
 sta current_room
 jsr loadLevel		;load level based on room
